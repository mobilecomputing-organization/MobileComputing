...
while time.time() < t_end:
    ...
    try:
        ...
        data = fulldata.split("#",2)[0]
        msg2 = fulldata.split("#",2)[1]
        startTime = fulldata.split("#",2)[2]
        ...
        if addr is not None and addr[0] != IP:
                ...
                if data[0] == RREQ:
                        if node not in data:
                                ...
                                s2 = RREP+data[1:]+":"+node
                                ss= UDP_IP[:-3]+data[-3:]
                                sock.sendto(RREP+data[1:]+":"+node+"#"+time.time(), (UDP_IP, UDP_PORT))
                        ...
                        if node in data:
                                ...
                                if node == data[2:5]:
                                        if data[2:] not in RList:
                                                RList.append(data[2:])
                                                print "path to ",data[-3:]," is ",data[2:],"time elaspsed ", str(time.time() - startTime) 
                                else:
                                        idx = data.find(node)
